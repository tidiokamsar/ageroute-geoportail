...
// Recherche globale multi-entites (code/nom/description/intitule). 5 resultats max par type.
searchRouter.get("/", requireAuth, async (req, res, next) => {
  try {
    ...
    const [troncons, ouvrages, pointsNoirs, postes, chantiers] = await Promise.all([
      ...
      prisma.pointNoir.findMany({ where: { ...notDeleted, description: contains }, select: { id: true, description: true }, take }),
      prisma.poste.findMany({ where: { ...notDeleted, nom: contains }, select: { id: true, nom: true }, take }),
      prisma.chantier.findMany({ where: { ...notDeleted, intitule: contains }, select: { id: true, intitule: true }, take }),
    ]);
